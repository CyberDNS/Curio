...
import { getFeeds, createFeed, updateFeed, deleteFeed, fetchFeeds } from '../../services/api'
import { Plus, Trash2, Loader2, RefreshCw, Download, Pencil } from 'lucide-react'
...
export default function FeedSettings() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Partial<Feed> }) => updateFeed(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['feeds'] })
      setEditingFeed(null)
    },
  ...
  const handleFetchConfirm = () => {
    ...
    fetchMutation.mutate({ feedId: fetchingFeedId || undefined, days: daysBack })
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        >
          {fetchingFeeds.size > 0 && !fetchingFeedId ? (
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="border border-newspaper-300 p-4 bg-newspaper-50">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setNewFeed({ ...newFeed, source_title: e.target.value })}
                className="w-full px-3 py-2 border border-newspaper-300 focus:outline-none focus:border-newspaper-900"
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 max-w-md w-full mx-4 border-2 border-newspaper-900">
            ...
            <p className="text-sm text-newspaper-600 mb-4">
              {fetchingFeedId
                ? `Fetch articles from "${feeds.find(f => f.id === fetchingFeedId)?.title || 'this feed'}"`
            ...
            <div className="mb-6">
              ...
              />
              <p className="text-xs text-newspaper-600 mt-1">
                Note: Most RSS feeds only provide recent articles (typically 10-50 items)
              </p>
            </div>
            <div className="flex gap-2">
              <button
        ...
        {feeds.map((feed) => (
          <div key={feed.id}>
            {editingFeed?.id === feed.id ? (
              // Edit form
              <form onSubmit={handleUpdate} className="border border-newspaper-300 p-4 bg-newspaper-50">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setEditingFeed({ ...editingFeed, source_title: e.target.value })}
                      className="w-full px-3 py-2 border border-newspaper-300 focus:outline-none focus:border-newspaper-900"
                      placeholder="New York Times"
                    />
                    <p className="text-xs text-newspaper-600 mt-1">
                  ...
                  <div>
                    <label className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) => setEditingFeed({ ...editingFeed, is_active: e.target.checked })}
                        className="w-4 h-4"
                      />
              ...
              <div className="flex justify-between items-center p-4 border border-newspaper-300 bg-white">
                <div className="flex-1">
                  <h4 className="font-semibold">{feed.title || 'Untitled Feed'}</h4>
                  {feed.source_title && (
                    <p className="text-sm text-newspaper-700 italic">Source: {feed.source_title}</p>
                  )}
                  <p className="text-sm text-newspaper-600 truncate">{feed.url}</p>
                  {feed.last_fetched && (
                ...
                <div className="flex gap-2">
                  ...
                  >
                    <RefreshCw
                      className={`w-4 h-4 ${fetchingFeeds.has(feed.id) ? 'animate-spin' : ''}`}
                    />
                  </button>
                  <button
                    onClick={() => {
