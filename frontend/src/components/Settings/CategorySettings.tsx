...
export default function CategorySettings() {
  ...
  const [newCategory, setNewCategory] = useState<CategoryCreate>({ name: '', slug: '', description: '' })
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                required
              />
              <p className="text-xs text-newspaper-600 mt-1">Used in URLs - lowercase, no spaces</p>
            </div>
            <div>
              ...
              <p className="text-xs text-newspaper-600 mt-1">
                Describe what content belongs in this category. The AI uses this to classify articles.
              </p>
            </div>
      ...
      <div className="space-y-2">
        {categories.map((category) => (
          <div key={category.id}>
            {editingCategory?.id === category.id ? (
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const name = e.target.value
                        setEditingCategory({ ...editingCategory, name, slug: generateSlug(name) })
                      }}
                      className="w-full px-3 py-2 border border-newspaper-300 focus:outline-none focus:border-newspaper-900"
                      required
                    />
              ...
              // Display mode
              <div className="flex justify-between items-start p-4 border border-newspaper-300 bg-white">
                <div className="flex-1">
                  <h4 className="font-semibold">{category.name}</h4>
                  <p className="text-sm text-newspaper-600">/{category.slug}</p>
                  {category.description && (
                    <p className="text-sm text-newspaper-700 mt-2 italic">"{category.description}"</p>
                  )}
                </div>
                <div className="flex gap-2">
                  <button
        ...
        {categories.length === 0 && (
          <p className="text-center text-newspaper-600 py-8">
            No categories added yet. Click "Add Category" to organize your feeds!
          </p>
        )}
      </div>
    </div>
