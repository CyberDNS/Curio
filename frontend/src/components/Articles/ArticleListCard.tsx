...
import { downvoteArticle, getProxiedImageUrl } from "../../services/api";
...
export default function ArticleListCard({
  ...
  const getArticleImage = () => {
    if (article.image_urls && article.image_urls.length > 0) {
      return getProxiedImageUrl(article.image_urls[0]);
    }
    if (article.image_url) {
      return getProxiedImageUrl(article.image_url);
    }
    return null;
  };
  ...
  return (
    <div className="border border-newspaper-300 hover:bg-newspaper-50 transition-colors flex gap-4">
      {/* Image */}
